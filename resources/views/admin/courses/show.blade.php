        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="flex flex-row justify-between items-center">
                    ...
                    <a href="{{ route('admin.course.add_video', $course) }}"
                        class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                        Add New Video
                    </a>
                </div>

                @forelse($course->course_videos as $videos)
                    <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                        <div class="flex flex-row items-center gap-x-3">
                            <iframe width="560" class="rounded-2xl object-cover w-[120px] h-[90px]" height="315"
                                src="https://www.youtube.com/embed/{{ $videos->path_video }}"
                                title="YouTube {{ $videos->name }}" frameborder="0"
                                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                                referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                            <div class="flex flex-col">
                                <h3 class="text-indigo-950 text-xl font-bold">{{ $videos->name }}</h3>
                                <p class="text-slate-500 text-sm">{{ $videos->course->name }}</p>
                            </div>
                        </div>


                        <div class="flex flex-row items-center gap-x-3">
                            <a href="{{ route('admin.course_videos.edit', $videos) }}"
                                class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                                Edit Video
                            </a>
                            <form action="{{ route('admin.course_videos.destroy', $videos) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="font-bold py-4 px-6 bg-red-700 text-white rounded-full">
