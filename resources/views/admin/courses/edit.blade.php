                ...
                <form method="POST" action="{{ route('admin.courses.update', $course) }}" enctype="multipart/form-data">
                        ...
                        <select name="category_id" id="category_id"
                            ...
                            @forelse($categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ $course->category->id == $category->id ? 'selected' : '' }}>{{ $category->name }}
                                </option>
                            @empty
                            @endforelse
                        </select>
                    ...
                    <div class="mt-4">
                        ...
                        <div class="flex flex-col gap-y-5">
                            ...
                            @foreach ($course->course_keypoints as $keypointText)
                                <input type="text" class="py-3 rounded-lg border-slate-300 border"
                                    placeholder="Write your copywriting" value="{{ $keypointText->name }}"
                                    name="course_keypoints[]">
                            @endforeach
