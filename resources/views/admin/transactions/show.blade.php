            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <div class="flex flex-row gap-x-10">
                    ...
                    <div class="flex flex-col gap-y-10">
                        ...
                        <div>
                            <p class="text-slate-500 text-sm my-2">Status</p>
                            @if (!$subscribeTransaction->subscription_start_date)

                            <span class="text-white text-sm font-bold rounded-full bg-orange-500 py-2 px-2 ">Pending</span>
                            @else
                            <span class="text-white text-sm font-bold rounded-full bg-green-500 py-2 px-2 ">Success</span>

                            @endif
                        </div>
                        <div>
                            <p class="text-slate-500 text-sm">Start Subcription Date</p>
                    ...
                    <div>
                        <img src="{{ Storage::url($subscribeTransaction->proof) }}" alt=" {{ $subscribeTransaction->user->name }}"
                        class="object-cover w-[300px] h-[550px]">
                    </div>
                </div>
                @if (!$subscribeTransaction->subscription_start_date)
